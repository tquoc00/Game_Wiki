...
import { Mail, Lock, User as UserIcon, Loader2, ArrowLeft, Gamepad2 } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      setError('Mật khẩu của bạn phải chứa tối thiểu 6 ký tự.');
      return;
    }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center p-4 bg-[#09090b] text-zinc-100 relative font-sans">
      ...
      <div className="w-full max-w-md space-y-8 text-left">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-zinc-400 uppercase tracking-wider font-medium">Tạo Tài Khoản Mới</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 md:p-8 space-y-6 relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-zinc-300 uppercase tracking-wide">Tên người dùng (Username)</label>
              <div className="relative">
                <input
                  type="text"
                  placeholder="Ví dụ: PlayerOne"
                  value={username}
                  ...
                  required
                />
                <UserIcon size={15} className="absolute left-3.5 top-3.5 text-zinc-500" />
              </div>
            </div>
          ...
          <div className="text-center text-xs text-zinc-400">
            Đã có tài khoản?{' '}
            <Link href="/login" className="font-bold text-cyan-400 hover:underline transition duration-200">
              Đăng nhập ngay
            </Link>
          </div>
        </div>
